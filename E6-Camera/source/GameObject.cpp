...
GameObject::GameObject() : mIndexCount(), mVertexStride() {}
...
void GameObject::Draw(ID3D11DeviceContext* deviceContext) {
  ...
  UINT offsets = 0;
  deviceContext->IASetVertexBuffers(0, 1, mVertexBuffer.GetAddressOf(),
                                    &strides, &offsets);
  ...
  deviceContext->VSGetConstantBuffers(CONST_BUFFER_MATERIAL_INDEX, 1, cBuffer.GetAddressOf());
  HR(deviceContext->Map(cBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedData));
  memcpy_s(mappedData.pData, sizeof(DT::CBMaterial), &mMaterial, sizeof(DT::CBMaterial));
  deviceContext->Unmap(cBuffer.Get(), 0);
...
void GameObject::SetDebugObjectName(const std::string& name) {
  ...
  D3D11SetDebugObjectName(mVertexBuffer.Get(), name + ".VertexBuffer");
  D3D11SetDebugObjectName(mIndexBuffer.Get(), name + ".IndexBuffer");
#else
