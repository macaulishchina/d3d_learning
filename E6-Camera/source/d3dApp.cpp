...
HINSTANCE D3DApp::AppInst() const { return mAppInst; }

HWND D3DApp::MainWnd() const { return mMainWnd; }
...
bool D3DApp::Init() {
  ...
  mMouseTracker = std::make_unique<DirectX::Mouse::ButtonStateTracker>();
...
void D3DApp::OnResize() {
  ...
  HR(mD3dDevice->CreateRenderTargetView(backBuffer.Get(), nullptr,
  ...
  HR(mD3dDevice->CreateDepthStencilView(mDepthStencilBuffer.Get(), nullptr,
                                        mDepthStencilView.GetAddressOf()));

  // 将渲染目标视图和深度/模板缓冲区结合到管线
  mD3dImmediateContext->OMSetRenderTargets(1, mRenderTargetView.GetAddressOf(),
  ...
  HRESULT hr = mD2dFactory->CreateDxgiSurfaceRenderTarget(
      surface.Get(), &props, mD2dRenderTarget.GetAddressOf());
  surface.Reset();
  if (hr == E_NOINTERFACE) {
    OutputDebugStringW(
        L"\n警告：Direct2D与Direct3D互操作性功能受限，你将无法看到文本信息。现"
        L"提供下述可选方法：\n"
        L"1. 对于Win7系统，需要更新至Win7 "
        L"SP1，并安装KB2670838补丁以支持Direct2D显示。\n"
        L"2. 自行完成Direct3D 10.1与Direct2D的交互。详情参阅："
        L"https://docs.microsoft.com/zh-cn/windows/desktop/Direct2D/"
        L"direct2d-and-direct3d-interoperation-overview"
        "\n"
        L"3. 使用别的字体库，比如FreeType。\n\n");
  } else if (hr == S_OK) {
    // 创建固定颜色刷和文本格式
    HR(mD2dRenderTarget->CreateSolidColorBrush(
        D2D1::ColorF(D2D1::ColorF::White), mColorBrush.GetAddressOf()));
    HR(mDwriteFactory->CreateTextFormat(
        L"宋体", nullptr, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL,
        DWRITE_FONT_STRETCH_NORMAL, 20, L"zh-cn", mTextFormat.GetAddressOf()));
  } else {
    // 报告异常问题
...
bool D3DApp::InitDirect3D() {
  ...
  UINT createDeviceFlags =
      D3D11_CREATE_DEVICE_BGRA_SUPPORT;  // Direct2D需要支持BGRA格式
  ...
  for (UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes;
    ...
    hr = D3D11CreateDevice(nullptr, d3dDriverType, nullptr, createDeviceFlags,
                           ...
                           mD3dDevice.GetAddressOf(), &featureLevel,
    ...
    if (hr == E_INVALIDARG) {
      ...
      hr = D3D11CreateDevice(
          nullptr, d3dDriverType, nullptr, createDeviceFlags, &featureLevels[1],
          numFeatureLevels - 1, D3D11_SDK_VERSION, mD3dDevice.GetAddressOf(),
          &featureLevel, mD3dImmediateContext.GetAddressOf());
    }
  ...
  // 检测 MSAA支持的质量等级
  mD3dDevice->CheckMultisampleQualityLevels(DXGI_FORMAT_B8G8R8A8_UNORM, 4,
                                            &m4xMsaaQuality);
  ...
  // DXGI工厂，否则会引发报错： "IDXGIFactory::CreateSwapChain: This function is
  ...
  if (dxgiFactory2 != nullptr) {
    ...
    fd.Windowed = TRUE;
    // 为当前窗口创建交换链
    HR(dxgiFactory2->CreateSwapChainForHwnd(mD3dDevice.Get(), mMainWnd, &sd,
                                            &fd, nullptr,
                                            mSwapChain1.GetAddressOf()));
    ...
    sd.SwapEffect = DXGI_SWAP_EFFECT_DISCARD;
    sd.Flags = 0;
    HR(dxgiFactory1->CreateSwapChain(mD3dDevice.Get(), &sd,
                                     mSwapChain.GetAddressOf()));
  }

  // 可以禁止alt+enter全屏
  dxgiFactory1->MakeWindowAssociation(
      mMainWnd, DXGI_MWA_NO_ALT_ENTER | DXGI_MWA_NO_WINDOW_CHANGES);
